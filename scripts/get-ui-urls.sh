...
echo ops-ui: http://$(kubectl get svc -A |grep xap-xap-manager-service |grep LoadBalancer |awk '{print $5}'):$( kubectl get svc -A |grep xap-xap-manager-service |grep LoadBalancer |awk '{print $6}'|cut -d':' -f1)
echo
